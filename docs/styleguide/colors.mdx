...
live — so toggling the theme switch shows both palettes. Spacing, type and
sizing tokens are in the table at the end. Swatch styling is inline on purpose,
to keep this page from adding anything to the production stylesheet.
...
`--ifm-color-primary` maps to `--src-primary` in light and `--src-highlight` in
dark. The `-dark` through `-lightest` variants are shades of `--src-primary` at
Docusaurus' own ±10/15/30% lightness steps; Infima uses `-dark` and `-darker`
for `.button--primary` hover and active.
