...
never touched, or a one-off literal with no brand variable behind it.
...
The green and grey ramps from the Source brand system, on the brand's own
step labels. `--src-green-400` is the same value as
`--src-primary`; `--ifm-color-primary` and its shades resolve through
`--src-green-*` in light mode. Everything grey in the site resolves back to
the grey ramp — the mapping onto Infima's `--ifm-color-gray-*` happens in
`variables.scss`. `--src-gray-50` has no Infima slot and is currently unused.
...
`--ifm-color-primary` maps to `--src-green-500` in light and `--src-highlight`
in dark — `--src-primary` (`--src-green-400`) only hits 2.8:1 on white, short
of the 4.5:1 text needs, so light mode anchors one step darker on the brand's
own green scale instead. The `-dark` through `-lightest` variants are also
`--src-green-*` steps, not a generated ramp; Infima uses `-dark` and `-darker`
